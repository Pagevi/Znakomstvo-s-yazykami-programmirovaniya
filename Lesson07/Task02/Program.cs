...
int[] FindPosition(int[,] matrix, int rowPosition, int columnPosition)
{
    int[] number = { 0, -1 };
...
void PrintAnswer(int[] results, int X, int Y)
{
    if (results[1] == -1) { Console.WriteLine("Такого элемента нет "); }
    else
    {
        Console.WriteLine($"Значение элемента в [{X}, {Y}]: {results[0]}");
    }
}
...
PrintAnswer(FindPosition(myArray, i, j), i, j);