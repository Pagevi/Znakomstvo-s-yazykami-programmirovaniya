...
int[] MinSumRow(int[,] matrix)
{
    ...
    RowNumberAndSum[1] = SumRow(matrix, RowNumberAndSum[0]);
    for (int i = 0; i < matrix.GetLength(0); i++)
    {
        if (RowNumberAndSum[1] > SumRow(matrix, i))
        {
            RowNumberAndSum[0] = i;
            RowNumberAndSum[1] = SumRow(matrix, RowNumberAndSum[0]);
        }
...
void Solve()
{
    ...
    Console.WriteLine($"Строка - {MinSumRow(newMatrix)[0] + 1} с наименьшей суммой элементов - {MinSumRow(newMatrix)[1]}");
}
